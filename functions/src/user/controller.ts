...
class UserController {
    ...
    static async holding (req: Request, res: Response) {
        try {
            ...
            await batch.commit();
            const memberRef = rtdb.ref(`groups/${holdingID}/members`);
    ...
    static async unHolding (req: Request, res: Response) {
        try {
            ...
            await batch.commit();
            const memberRef = rtdb.ref(`groups/${unHoldingID}/members`);
            const memberRemoveRef = memberRef.child(userID);
            await memberRemoveRef.remove();
            res.status(200).json({
